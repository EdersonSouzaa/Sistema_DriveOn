...
const carros = [
  { id: 1, nome: "Jeep Compass", placa: "ABC-1234", detalhes: "SUV, URBANO, 4 Portas", preco: 90, status: "Disponível", imagem: "./assets/cars/car_card1.jpg" },
  { id: 2, nome: "Maserati GranTurismo", placa: "DEF-5678", detalhes: "Luxo, Esportivo, 2 Portas", preco: 150, status: "Disponível", imagem: "./assets/cars/car_card2.jpg" },
  { id: 3, nome: "Mini Cooper", placa: "GHI-9012", detalhes: "Popular econômico 4 portas", preco: 200, status: "Disponível", imagem: "./assets/cars/car_card3.jpg" },
  { id: 4, nome: "BMW X6", placa: "JKL-3456", detalhes: "Luxo, Esportivo, 4 Portas", preco: 120, status: "Disponível", imagem: "./assets/cars/car_card4.jpg" },
  { id: 5, nome: "McLaren 720S", placa: "MNO-7890", detalhes: "Luxo, Esportivo, 2 Portas", preco: 180, status: "Disponível", imagem: "./assets/cars/car_card5.png" },
  { id: 6, nome: "Range Rover Evoque", placa: "PQR-1234", detalhes: "SUV, Urbano, 4 Portas", preco: 160, status: "Disponível", imagem: "./assets/cars/car_card6.png" }
];
...
// CORREÇÃO: Lógica de filtros exatos e busca por modelo
function applyFilters() {
  ...
  const filtrados = carros.filter(carro => {
    ...
    const carroPlaca = carro.placa.toUpperCase();
    const carroDetalhes = carro.detalhes.toLowerCase();

    // Modelo: Busca se o nome contém o que foi selecionado
    const matchModel = !model || carroNome.includes(model);
    
    // Placa: Busca Exata (compara exatamente o texto)
    const matchPlate = !plate || carroPlaca === plate;
    
    // Categoria: Busca se nos detalhes do carro contém a categoria exata (ex: 'SUV')
    const matchCategory = !category || carroDetalhes.includes(category);
...
// Extra: Filtro de busca em tempo real no input de pesquisa principal
document.getElementById("searchInput")?.addEventListener("input", (e) => {
  const valor = e.target.value.toLowerCase();
  const filtrados = carros.filter(carro => 
    carro.nome.toLowerCase().includes(valor) || 
    carro.placa.toLowerCase().includes(valor) ||
    carro.detalhes.toLowerCase().includes(valor)
  );
  renderizarCarros(filtrados);
});